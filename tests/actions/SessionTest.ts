...
import reauthenticate from '@src/libs/Reauthentication';
...
describe('Session', () => {
    ...
    describe('SAML reauthentication', () => {
        test('reauthenticate redirects to sign in when isSAMLRequired=true and no auto-generated credentials exist', async () => {
            // Given a SAML user with no auto-generated credentials stored
            await Onyx.set(ONYXKEYS.ACCOUNT, {isSAMLRequired: true} as Account);
            await waitForBatchedUpdates();

            const redirectToSignInSpy = jest.spyOn(SignInRedirect, 'default').mockImplementation(() => Promise.resolve());

            // When reauthenticate is triggered (e.g. by a 407 NOT_AUTHENTICATED response)
            const result = await reauthenticate('TestSAMLNoCredentials');
            await waitForBatchedUpdates();

            // Then it should redirect through the SAML browser flow, not attempt silent Authenticate
            expect(result).toBe(false);
        ...
        test('reauthenticate does NOT redirect to sign in when isSAMLRequired=true but auto-generated credentials exist', async () => {
            // Given a SAML user who has already authenticated and has auto-generated credentials
            await Onyx.set(ONYXKEYS.ACCOUNT, {isSAMLRequired: true} as Account);
            await Onyx.set(ONYXKEYS.CREDENTIALS, {
                login: 'test@saml-domain.com',
                autoGeneratedLogin: 'auto-login-token',
                autoGeneratedPassword: 'auto-password-token',
            });
            await waitForBatchedUpdates();

            const redirectToSignInSpy = jest.spyOn(SignInRedirect, 'default').mockImplementation(() => Promise.resolve());

            // When a background API call triggers reauthentication (e.g. transient 407)
            // Mock the Authenticate network call to return a successful response
            HttpUtils.xhr = jest.fn<typeof HttpUtils.xhr<never>>().mockResolvedValue({
                jsonCode: 200,
                authToken: 'new-auth-token',
            } as never);

            const result = await reauthenticate('TestSAMLWithCredentials');
            await waitForBatchedUpdates();

            // Then it should NOT redirect to sign in — it should fall through to the silent Authenticate call
            expect(redirectToSignInSpy).not.toHaveBeenCalled();
            // And the Authenticate network call should have been made
            expect(HttpUtils.xhr).toHaveBeenCalledWith(
                'Authenticate',
                expect.objectContaining({
                    partnerUserID: 'auto-login-token',
                    partnerUserSecret: 'auto-password-token',
                }),
                'post',
                false,
                undefined,
