...
function ReportActionsView({reportID, onLayout}: ReportActionsViewProps) {
    ...
    // isLoadingOnyxValue guard ensures report?.lastReadTime has hydrated before the session
    // starts. Without it, on cold open the REPORT_<id> key can be loading while cached actions
    // already exist, causing startSession to fire with lastReadTime=undefined and locking
    // sessionStartTime to now — hiding the unread Concierge message behind Show full history.
    // On a warm cache reportResult.status is 'loaded' immediately, so behaviour is unchanged.
    const canStartConciergeSession = !isLoadingOnyxValue(reportResult) && (!!hasOnceLoadedReportActions || allReportActions.length > 0);
